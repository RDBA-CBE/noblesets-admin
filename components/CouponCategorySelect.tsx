...
export default function CouponCategorySelect({
    queryFunc,
    ...
    const categoryList = async (query = '', after = null) => {
        ...
        const exceptGift = formattedOptions?.filter((option) => option.label != 'Gift Card');
        const pageInfo = res?.data?.categories?.pageInfo;
        setState({
            catOption: after ? [...state.catOption, ...exceptGift] : exceptGift,
            ...
            <Select
                ...
                styles={{
                    ...
                    option: (base, state) => ({
                        ...
                        backgroundColor: state.isDisabled ? '#f0f0f0' : state.isSelected ? '#fff' : state.isFocused ? '#eee' : '#fff',
                        cursor: 'pointer',
                    }),
