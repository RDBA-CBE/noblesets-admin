...
const CreateCoupon = () => {
    ...
    const extractShippingPrices = (data) => {
        const result = {
            standardShippingRupee: null,
            standardShippingDollar: null,
            expressShippingRupee: null,
            expressShippingDollar: null,
        };

        const channels = data?.shippingZone?.channels || [];

        channels.forEach((channel) => {
            channel.availableShippingMethodsPerCountry.forEach((countryEntry) => {
                countryEntry.shippingMethods.forEach((method) => {
                    const { name, price } = method;

                    if (price.currency === 'USD') {
                        if (name === 'Standard Shipping' && result.standardShippingDollar === null) {
                            result.standardShippingDollar = price.amount;
                        }
                        if (name === 'Express Shipping' && result.expressShippingDollar === null) {
                            result.expressShippingDollar = price.amount;
                        }
                    }

                    if (price.currency === 'INR') {
                        if (name === 'Standard Shipping' && result.standardShippingRupee === null) {
                            result.standardShippingRupee = price.amount;
                        }
                        if (name === 'Express Shipping' && result.expressShippingRupee === null) {
                            result.expressShippingRupee = price.amount;
                        }
                    }
                });
            });
    ...
    const getDetails = async () => {
        try {
            ...
            const jsonResponse = res; // Replace with your actual GraphQL response
            const prices = extractShippingPrices(jsonResponse.data);
    ...
    const createShippingZone = async () => {
        try {
            ...
            const body = {
                name: state.zoneName,
                countries: [state.selectedCountry?.value],
                default: false,
